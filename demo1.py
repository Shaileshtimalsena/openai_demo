# ===============================================================
#  ESAG Art Hub – AI-Powered Prototype (Final Version with AI Recommendations)
# ===============================================================
...
def recommend_artworks_with_openai(query, artworks):
    """Use OpenAI to recommend artworks matching buyer description."""
    if not query:
        return None
    ...
    Analyze and rank which 3 artworks best fit the buyer's description.
...
with home_tab:
    ...
    if query and openai.api_key:
        with st.spinner("Finding best matches using OpenAI..."):
            recommendations = recommend_artworks_with_openai(query, ARTWORKS)
        if recommendations:
            st.success("**AI Recommendations:**")
            st.write(recommendations)
        else:
            st.warning("No recommendations available. Try another description.")
    else:
        st.info("Type something in the search bar to get AI-based art suggestions.")

    # ---------- Full Gallery ----------
    st.markdown("### All Artworks")
    cols = st.columns(3)
